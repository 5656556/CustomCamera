...
@property (nonatomic, strong)AMapLocationManager *locationAMapManager;
...
- (AMapLocationManager *)locationAMapManager {
    if (!_locationAMapManager) {
        _locationAMapManager = [[AMapLocationManager alloc]init];
        _locationAMapManager.delegate = self;
    }
    return _locationAMapManager;
}
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.showSheetSwitch.on = YES;
    self.view.backgroundColor = [UIColor whiteColor];
    _selectedPhotos = [NSMutableArray array];
    _selectedAssets = [NSMutableArray array];
    [self configCollectionView];
    CLLocation *loc = [[CLLocation alloc]initWithLatitude:33.995698 longitude:115.262302];
    [self checkCLL:loc];
    [self.locationManager requestLocationWithReGeocode:YES withNetworkState:YES completionBlock:^(BMKLocation * _Nullable location, BMKLocationNetworkState state, NSError * _Nullable error) {
        NSLog(@"baiduxxx: \n1. %@\n2.%@\n3.%@\n4.%@\n5.%@ \n",
              ...
              location.rgcData.city,
              location.rgcData.district,
              location.rgcData.street,
              location.rgcData.locationDescribe
              );
        
        [location.rgcData.poiList enumerateObjectsUsingBlock:^(BMKLocationPoi * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSLog(@"\n%@",obj.name);
        }];
    ...
    //   定位超时时间，最低2s，此处设置为2s
    self.locationAMapManager.locationTimeout = 2;
    //   逆地理请求超时时间，最低2s，此处设置为2s

    self.locationAMapManager.reGeocodeTimeout = 2;
    [self.locationAMapManager requestLocationWithReGeocode:YES completionBlock:^(CLLocation *location, AMapLocationReGeocode *regeocode, NSError *error) {
        if (error)
        {
    ...
    if (indexPath.item == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] containsString:@"GIF"] && self.allowPickingGifSwitch.isOn && !self.allowPickingMuitlpleVideoSwitch.isOn) {
            ...
            [imagePickerVc setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
                NSLog(@"zhaopianyuan:%@, photos:%@",assets,photos);
...
//    }
    ...
    PGG_SelfieViewController *fieVC = [[PGG_SelfieViewController alloc] init];
    fieVC.modalTransitionStyle = UIModalPresentationOverCurrentContext;
    fieVC.modalPresentationStyle = UIModalPresentationFullScreen;
    fieVC.delegate = self;
    [self presentViewController:fieVC animated:YES completion:nil];
    return;
    ...
    cameraVC.callback = ^(NSArray *cameras){
        ...
        ZLCamera *canamerPhoto = cameras[0];
        UIImage *image = canamerPhoto.photoImage;
        NSArray * arr = @[@"09:39 2019-08-08 星期一",@"中关村新兴产业联盟大厦·北京市",@"uploader"];
        UIImage *img  = [image imageWaterMarkWithString:arr startPoint:CGPointMake(15, 13)];
        ...
        if (img) {
            image = img;
            dispatch_async(dispatch_get_main_queue(), ^{
                [[TZImageManager manager] savePhotoWithImage:image meta:nil location:self.location completion:^(PHAsset *asset, NSError *error){
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    __block __weak UIImagePickerController *pickerVC = picker;
...
- (void)checkCLL:(CLLocation *)location {
    ...
    [clGeoCoder reverseGeocodeLocation:cl completionHandler: ^(NSArray *placemarks,NSError *error) {
        ...
        for (CLPlacemark *placeMark in placemarks) {
            ...
            NSLog(@"所在城市====%@ %@ %@ %@  %@  %@", state, city, subLocality, street,placeMark.postalCode,placeMark.areasOfInterest);
        }
    }];
}
...
- (void)imagePickerController:(TZImagePickerController *)picker didFinishPickingPhotos:(NSArray<UIImage *> *)photos sourceAssets:(NSArray *)assets isSelectOriginalPhoto:(BOOL)isSelectOriginalPhoto infos:(NSArray<NSDictionary *> *)infos {
    ...
    for (PHAsset *phAsset in assets) {
        NSLog(@"asset:%@ location:%@",phAsset,phAsset.location);
        if (phAsset.location) {
            [self checkCLL:phAsset.location];
        }
    }
...
- (void)mediaCaptureController:(UIViewController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    id type = info[UIImagePickerControllerMediaType];
    if ([type isEqualToString:@"public.image"]) {
        UIImage *image = [info objectForKey:UIImagePickerControllerOriginalImage];
//        image = [UIImage imageWithData:UIImageJPEGRepresentation(image, 0.5)];
//        if (image.size.width>self.view.frame.size.width*1.2) {
//            image = [image imageFitInSize:CGSizeMake(self.view.frame.size.width*2, image.size.height*self.view.frame.size.width*2/image.size.width)];
//        }
...
- (void)saveImage:(UIImage *)image {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (img) {
            dispatch_async(dispatch_get_main_queue(), ^{
                [[TZImageManager manager] savePhotoWithImage:img meta:nil location:self.location completion:^(PHAsset *asset, NSError *error){
                    if (error) {
                        NSLog(@"图片保存失败 %@",error);
                    } else {
                        self.imagePickerVc = nil;
                        TZAssetModel *assetModel = [[TZImageManager manager] createModelWithAsset:asset];
                        if (self.allowCropSwitch.isOn) { // 允许裁剪,去裁剪
                            TZImagePickerController *imagePicker = [[TZImagePickerController alloc] initCropTypeWithAsset:assetModel.asset photo:img completion:^(UIImage *cropImage, id asset) {
                                [self refreshCollectionViewWithAddedAsset:asset image:cropImage];
                            }];
                            imagePicker.allowPickingImage = YES;
                            imagePicker.needCircleCrop = self.needCircleCropSwitch.isOn;
                            ...
                            [self refreshCollectionViewWithAddedAsset:assetModel.asset image:img];
                        }
                    }
                }];
            });

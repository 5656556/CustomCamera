...
- (void)initialize {
    ...
    CGFloat topY =  statusRect.size.height;
    self.preview =[AVCaptureVideoPreviewLayer layerWithSession:self.session];
    self.preview.videoGravity = AVLayerVideoGravityResizeAspectFill;
    self.preview.frame = CGRectMake(0, 40+topY,viewWidth, viewHeight);
    
    NSLog(@"%@",NSStringFromCGRect(self.view.frame));
    ZLCameraView *caramView = [[ZLCameraView alloc] initWithFrame:CGRectMake(0, 40+topY, viewWidth, viewHeight)];
...
- (UIButton *)setupButtonWithImageName:(NSString *)imageName andX:(CGFloat )x {
    ...
    button.frame = CGRectMake(x, 20, 80, self.topView.frame.size.height-20);
...
- (void)setup {
    ...
    topView.frame = CGRectMake(0, 0, self.view.frame.size.width, 40+topY);

...
@interface PGG_SelfieViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,CAAnimationDelegate,LGCameraImageViewDelegate>

//@property(nonatomic,strong)HXPhotoManager * manager;
@property (nonatomic, strong) LGCameraImageView *previewView;
...
//         [self.previewBtn setBackgroundImage:self.previewImage forState:UIControlStateNormal];
         //代理回传
     NSDictionary *dic = @{UIImagePickerControllerOriginalImage:image,
                           UIImagePickerControllerMediaType:@"public.image"
                           };
         self.nextDic = dic;
         [self displayImages:self.previewImage withUrl:nil];
     }];
}
#pragma mark - 视频录制事件
- (void)videoAction {
...
//    [self.previewBtn setBackgroundImage:_previewImage forState:UIControlStateNormal];
    ...
    self.nextDic = dic;
    [self displayImages:nil withUrl:outputFileURL];
}
...
- (void)displayImages:(UIImage *)images withUrl:(NSURL *)url{
    
    if (!self.previewView) {
        LGCameraImageView *view = [[LGCameraImageView alloc] initWithFrame:self.view.frame];
        view.delegate = self;
        view.imageOrientation = XGImageOrientationUp;
        self.previewView = view;
    }
    if(![self.view.subviews containsObject:self.previewView]) {
        [self.view addSubview:self.previewView];
    }
    if (images) {
        self.previewView.imageToDisplay = images;
    }else if (url) {
        self.previewView.movieURL = url;
    }
...
- (void)xgCameraImageViewSendBtnTouched {
    if ([self.delegate respondsToSelector:@selector(mediaCaptureController:didFinishPickingMediaWithInfo:)]) {
        [self.delegate mediaCaptureController:self didFinishPickingMediaWithInfo:self.nextDic];
    }
